...
FILE_PATH="/Users/empid016/Documents/Ravi/ravi_zoho/website/src/components/dashboard/AdminOverview.tsx"
...
import {
    ...
    FiPlus, FiFileText, FiSend, FiInbox, FiActivity, FiUserPlus, FiMapPin, FiAlertTriangle, FiCheckCircle
} from "react-icons/fi";
import {
...
export default function AdminOverview({ data, userName, filter, setFilter, onRefresh }: AdminOverviewProps) {
    ...
    const attendanceTrends = (data.attendanceTrends || []).map((d: any) => ({
        ...
        Present: d.present,
        Absent: Math.max(0, (data.employees?.active || 48) - d.present) // Fallback to 48 for matching mockup
    }));
